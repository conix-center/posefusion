...
def repro_error(pts4D, proj0, proj1, pt1, pt2):
    ...
    # error = np.sqrt(np.power(pt1_est[:,0] - pt1.T[:,0], 2)+np.power(pt1_est[:,1] - pt1.T[:,1], 2))
    ...
    # Obtain error for each point 
    # error2 = np.sqrt(np.power(repro2[:,0] - pt2.T[:,0], 2)+np.power(repro2[:,1] - pt2.T[:,1], 2))
...
def check_body_length(body):
    ...
    if (dist_neck_l_eye > 1):
        body[15] = 0

    if (dist_neck_r_eye > 1):
        body[16] = 0
    ...
    if ((r_arm > 1) or (r_arm <= 0)):
        # print("Right arm length incorrect")
        body[2] = 0
        body[3] = 0
        body[4] = 0
    ...
    if ((l_arm > 1) or (l_arm <= 0)):
        # print("Left arm length incorrect")
        body[5] = 0
        body[6] = 0
        body[7] = 0
    ...
    if ((l_upper_leg > 1) or (l_upper_leg <= 0)):
        # print("Left upper leg length incorrect")
        body[9] = 0
        body[10] = 0
    
    if ((l_lower_leg > 1) or (l_lower_leg <= 0)):
        # print("Left lower leg length incorrect")
        body[10] = 0
        body[11] = 0
             
    # Check right leg lengths
    if ((r_upper_leg > 1) or (r_upper_leg <= 0)):
        # print("Right upper leg length incorrect")
        body[12] = 0
        body[13] = 0
    if ((r_lower_leg > 1) or (r_lower_leg <= 0)):
        # print("Right lower leg length incorrect")
        body[13] = 0
        body[14] = 0

    # print("{:.2f} {:.2f}".format(r_foot, l_foot))
       
    # Check foot length
    if (r_foot > 0.5) or (r_foot <= 0):
        # print("Right foot length incorrect")       
        body[22] = 0
        body[24] = 0
        body[11] = 0
        body[23] = 0
        
    if (l_foot > 0.5) or (l_foot <= 0):
        # print("Left foot length incorrect")       
        body[14] = 0
        body[19] = 0
        body[20] = 0
        body[21] = 0
    ...
    for body_num_cam_ref in range(num_bodies_ref_cam):
        ...
        for camera_num in list_cams:
            for body_num in range(get_num_body_camera(camera_num)):
                ...
                # print(camera_num, body_num, new_error)
...
def find_new_mapping(prev, new, numBodies, prev_mapping):
    ...
    # Set previous bodies to be new
    # prev = new
    # print("upd")
    # print(prev_updated[:5, :, 0])
...
def on_message(client, userdata, msg):
    ...
    # Obtain array
    # points = np.vstack(pts1_calib)


        


# Take a string and returns the number id of the person along with an array of its coordinates
...
if __name__ == '__main__':
    ...
    K1 = K2 = np.load("calib1.npz")["mtx"]     
    # K2 = np.load("calib2.npz")["mtx"]   
    ...
    # Compute new projection matrices
        ...
        np.savez("../results/projections.npz", num_cameras = NUM_CAMERAS, K1=K1, K2=K2, projs=projs, m_matrices = m_matrices,date=int(time.time()))
    ...
    while True:
        ...
        if (received_data[0] and received_data[1] and received_data[2]):
            # print(received_data[0]-received_data[1])
                ...
                for person in range(numBodies):                
                    ...
                    if (frame == FRAME_AVERAGING):
                        ...
                        if (nz > MIN_BODY_POINTS):
                            ...
                            print("[{}] Person{}[{}]: NZ parts = {} ------------- SENDING".format(time.time()*1000000 % 10000000, person, id_mapping[person], nz))
    ...
    # # Data collection
    # index = ["nose", "neck", "rshoulder", "relbow", "rwirst", "lshoulder", "lelbow", "lwirst", "midhip", "rhip", "rknee", "rankle", "lhip", "lknee", "lankle", "reye", "leye", "rear", "lear", "lbigtoe", "lsmalltoe", "lheel", "rbigtoe", "rsmalltoe", "rheel"]
    # columns = ["body_num", "x", "y", "z"]
...
#     df2.loc[:, "unique_num"] = unique_num
#     df2.loc[:, "person_num"] = person
#     df2.loc[:, "body_num"] = id_mapping[person]
#     df2.loc[:, ["x", "y", "z"]] = points3D[:,:,id_mapping[person]]
#     df= df.append(df2)
#     if (unique_num > 100 and exported == False):
#         df.to_csv("bodies.csv")
